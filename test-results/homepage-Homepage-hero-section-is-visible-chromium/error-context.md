...
- Name: homepage.spec.ts >> Homepage >> hero section is visible
- Location: e2e\homepage.spec.ts:20:3
...
Locator: locator('h1').filter({ hasText: /próximo|hogar|Córdoba/i }).first()
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('h1').filter({ hasText: /próximo|hogar|Córdoba/i }).first()
