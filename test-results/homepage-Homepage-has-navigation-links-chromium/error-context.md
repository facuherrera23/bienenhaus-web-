...
- Name: homepage.spec.ts >> Homepage >> has navigation links
- Location: e2e\homepage.spec.ts:11:3
...
Locator: locator('header a, nav a, .ds-header a').filter({ hasText: /Propiedades|Agentes|Contacto|Nosotros/i }).first()
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for locator('header a, nav a, .ds-header a').filter({ hasText: /Propiedades|Agentes|Contacto|Nosotros/i }).first()
