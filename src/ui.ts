...
export function initDetalleModal() {
  ...
  contactBtn?.addEventListener('click', async () => {
    const { propiedadActual, formatearPrecio } = await import('./properties.ts');
    if (propiedadActual) {
...
export function initFiltros() {
  document.getElementById('btnFiltrar')?.addEventListener('click', async () => {
    const { aplicarFiltros } = await import('./properties.ts');
    await aplicarFiltros();
  });

  document.getElementById('btnLimpiar')?.addEventListener('click', async () => {
    const { limpiarFiltros } = await import('./properties.ts');
    limpiarFiltros();
  });
...
let CONFIG = null;
import('./config.ts').then(m => { CONFIG = m.CONFIG; });
