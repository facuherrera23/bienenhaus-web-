...
import { sanitizeText, sanitizeHtml } from '../../../utils/sanitize.ts';
...
function renderHeroTitulo(value: any) { const el = document.getElementById('heroTitulo'); if (el) el.innerHTML = sanitizeHtml(value).replace(/\n/g, '<br>'); }
function renderHeroSubtitulo(value: any) { const el = document.getElementById('heroSubtitulo'); if (el) el.textContent = value; }
function renderHeroBadges(value: any) { const container = document.getElementById('heroBadges'); if (!container) return; const badges = Array.isArray(value) ? value : value.split('\n').filter((b: string) => b.trim()); container.innerHTML = badges.map((badge: string) => `<span class="hero-badge"><i class="fas fa-check-circle"></i> ${sanitizeText(badge.trim())}</span>`).join(''); }
function renderHeroCtaPrimario(value: any) { const el = document.getElementById('heroCtaPrimario'); if (el) el.innerHTML = `<i class="fas fa-search"></i> ${sanitizeText(value)}`; }
function renderHeroCtaSecundario(value: any) { const el = document.getElementById('heroCtaSecundario'); if (el) el.innerHTML = `<i class="fas fa-comment-dots"></i> ${sanitizeText(value)}`; }
function renderHeroStats(value: any) {
  ...
  container.innerHTML = stats.map(stat => `
    <div class="stat"><div class="number" style="font-size: 2.5rem; font-weight: 800; color: var(--primary);">${sanitizeText(stat.valor || stat.valor === 0 ? stat.valor : '—')}</div><div class="label">${sanitizeText(stat.label || '')}</div></div>
  `).join('');
}
...
function renderAboutDescripcion(value: any) { const el = document.getElementById('aboutDescripcion'); if (el) el.innerHTML = sanitizeHtml(value).replace(/\n/g, '<br>'); }
function renderAboutValores(value: any) {
  ...
  container.innerHTML = valores.map(v => `
    <div class="valor-item"><i class="${sanitizeAttr(v.icono) || 'fas fa-star'}" aria-hidden="true"></i><h4>${sanitizeText(v.titulo || '')}</h4><p>${sanitizeText(v.descripcion || '')}</p></div>
  `).join('');
}
...
function renderServiciosLista(value: any) {
  ...
  container.innerHTML = servicios.map(s => `
    <div class="servicio-item"><i class="${sanitizeAttr(s.icono) || 'fas fa-home'}" aria-hidden="true"></i><h4>${sanitizeText(s.titulo || '')}</h4><p>${sanitizeText(s.descripcion || '')}</p></div>
  `).join('');
}
...
// ================================================================
function renderPorQueTitulo(value: any) { const el = document.getElementById('porQueTitulo'); if (el) el.innerHTML = `<i class="fas fa-star"></i> ${sanitizeText(value)}`; }
...
function renderPorQueRazones(value: any) {
  ...
  container.innerHTML = razones.map(r => `
    <div class="porque-item"><div class="icono" style="font-size:2.5rem;">${sanitizeText(r.emoji || '⭐')}</div><h4>${sanitizeText(r.titulo || '')}</h4><p>${sanitizeText(r.descripcion || '')}</p></div>
  `).join('');
}
...
// ================================================================
function renderEquipoTitulo(value: any) { const el = document.getElementById('equipoTitulo'); if (el) el.innerHTML = `<i class="fas fa-users"></i> ${sanitizeText(value)}`; }
...
// ================================================================
function renderOficinasTitulo(value: any) { const el = document.getElementById('oficinasTitulo'); if (el) el.innerHTML = `<i class="fas fa-map-marker-alt"></i> ${sanitizeText(value)}`; }
...
// ================================================================
function renderFooterMarca(value: any) { const el = document.getElementById('footerMarca'); if (el) el.innerHTML = sanitizeHtml(value); }
function renderFooterDescripcion(value: any) { const el = document.getElementById('footerDescripcion'); if (el) el.textContent = value; }
function renderFooterContacto(value: any) { const el = document.getElementById('footerContacto'); if (el) el.innerHTML = sanitizeHtml(value).replace(/\n/g, '<br>'); }
function renderFooterLinks(value: any) {
  ...
  container.innerHTML = links.map(l => `<li><a href="${sanitizeUrl(l.url) || '#'}"><i class="fas fa-chevron-right"></i> ${sanitizeText(l.texto || '')}</a></li>`).join('');
}
function renderFooterServicios(value: any) {
  ...
  container.innerHTML = servicios.map(s => `<li><a href="${sanitizeUrl(s.url) || '#'}"><i class="fas fa-chevron-right"></i> ${sanitizeText(s.texto || '')}</a></li>`).join('');
}
...
function renderSeoSchema(value: any) { const schema = document.querySelector('script[type="application/ld+json"]'); if (schema && value) { try { schema.textContent = sanitizeText(value); } catch (e) { logWarn('Schema JSON inválido', { error: e }, 'admin-content'); } } }
...
function renderSiteEslogan(value: any) { const badge = document.getElementById('heroBadgeTop'); if (badge) badge.innerHTML = `<i class="fas fa-crown"></i> ${sanitizeText(value)}`; }
...
// RENDERIZADORES - FAQ
// ================================================================
function renderFaqTitulo(value: any) { const el = document.getElementById('faqTitulo'); if (el) el.innerHTML = `<i class="fas fa-question-circle"></i> ${sanitizeText(value)}`; }
...
function renderFaqGrid(value: any) {
  ...
  container.innerHTML = faqs.map(faq => `<div class="faq-item"><h4><i class="fas fa-chevron-right"></i> ${sanitizeText(faq.pregunta || '')}</h4><p>${sanitizeText(faq.respuesta || '')}</p></div>`).join('');
}

// ================================================================
// RENDERIZADORES - CONTACTO
// ================================================================
function renderContactoTitulo(value: any) { const el = document.getElementById('contactoTitulo'); if (el) el.innerHTML = `<i class="fas fa-envelope"></i> ${sanitizeText(value)}`; }
